...
from model.exps.utils import FinishException
from model.ga.ga import GAPipeline
from model.rl.rl import RLPipeline
from model.rl.utils import get_expresssion_and_reward
...
class Pipeline:
    def __init__(self, config: Config = None):
        ...
        if config is None:
            self.config = Config()
            self.config.init()
        self.ga1, self.ga2 = GAPipeline(config), GAPipeline(config)
        self.rl1, self.rl2 = RLPipeline(config), RLPipeline(config)
        self.msdb = MSDB(config_s=config)
        self.expr_form = ""
    ...
    def fit(self, x=None, t=None, x_test=None, t_test=None):
        ...
        try:
            ...
            for tms in range(self.config.epoch):
                ...
                if tms % 10 <= 8:
                    self.rl1.run()
                    pop = self.ga1.ga_play(self.rl1.get_status())
                    sym_tol1 += pop
                    self.change_expr_form(pop)
                if tms % 10 >= 5:
                    self.rl2.run()
                    pop = self.rl2.get_status()
                    sym_tol2 += pop
    ...
    def change_expr_form(self, pops):
        ...
        symbols = [get_expresssion_and_reward(self.rl2.agent, pop, self.config) for pop in set(pops)]
        symbols_count = Counter(pops)
        self.expr_form = self.msdb.get_form(symbols, symbols_count)
        if self.expr_form:
            self.rl2.agent.change_exp(self.expr_form)
            self.ga2.agent.change_exp(self.expr_form)
